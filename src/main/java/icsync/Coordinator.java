...
public class Coordinator {
    ...
    LoggingScheduledThreadPoolExecutor addressLookupExecutor;
    DatagramChannel ch;
    Selector sel;
    Thread selectThread;
    ...
    void init(InetSocketAddress address) {
        ...
        addressLookupExecutor = new LoggingScheduledThreadPoolExecutor(1, logger);
    ...
    void addConf(String arg) {
        ...
        if (cfgId != configurations.size()) {
            logger.warn("Tried to add configuration " + cfgId + " but configuration " + configurations.size()
                    + " is the next free configuration");
            return;
        }
    ...
    void processMultipartResponse(InetSocketAddress address, ByteBuffer res) {
        ...
        // Update information about neighbor's state.
        if (mres.containsRestarts && cs.restarts < mres.restarts) {
            cs.restarts = mres.restarts;

            cs.mrho = RoundId.ZERO;
            cs.arbb = RoundId.ZERO;
            cs.nextUnknownCfg = 0;
        }

        if (mres.mrho != null && cs.mrho.compareTo(mres.mrho) < 0) {
            cs.mrho = mres.mrho;
        }

        if (mres.arbb != null && cs.arbb.compareTo(mres.arbb) < 0) {
            cs.arbb = mres.arbb;
        }

        if (mres.containsNextUnknownCfgId && cs.nextUnknownCfg < mres.nextUnknownCfgId) {
            cs.nextUnknownCfg = mres.nextUnknownCfgId;
        }

        // Update information about mrho and arbb.
        ...
        boolean reschedule = cs.nextUnknownCfg < configurations.size() || cs.mrho.compareTo(mrho) < 0 || cs.arbb.compareTo(arbb) < 0;
